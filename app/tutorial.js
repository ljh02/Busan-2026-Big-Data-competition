...
(function () {
  ...
  const TUTORIAL = [
    ...
    },
    {
      ...
      points: [
        { head: "조건 입력", body: "예산, 회사 주소, 가구 유형과 통근·주거비·SOC·안전 가중치를 내 기준대로 조절해요." },
    ...
    },
    {
      ...
      points: [
        ...
        { head: "AI Agent", body: "\"이 아파트 전세 들어가도 괜찮아?\" 같은 질문에 가격·통근·위험·입지 근거를 나눠 답해요." },
  ...
  function renderPoints(s) {
    ...
    const items = s.points.map((p, i) => `
      <li>
        ...
        <span><strong>${esc(p.head)}</strong><span class="tut-body"> — ${esc(p.body)}</span></span>
      </li>`).join("");
